...
class CachedDataService {
  ...
  static final _reportsController = StreamController<List<Map<String, dynamic>>>.broadcast();
  static final _notificationsController = StreamController<List<Map<String, dynamic>>>.broadcast();
  static final _activitiesController = StreamController<List<Map<String, dynamic>>>.broadcast();
  static final _statisticsController = StreamController<Map<String, int>>.broadcast();
  ...
  static Stream<List<Map<String, dynamic>>> getReportsStream({
    ...
    _reportsSubscription = query.snapshots().listen((snapshot) {
      if (snapshot.docs.isNotEmpty) {
        ...
        print('🔄 Updated ${reports.length} reports from Firestore and cached');
      }
    }, onError: (error) {
  ...
  static Stream<List<Map<String, dynamic>>> getNotificationsStream() {
    ...
    if (cachedNotifications != null && cachedNotifications.isNotEmpty) {
      ...
      print('⚡ Emitted ${cachedNotifications.length} notifications from cache INSTANTLY');
    } else {
    ...
    _notificationsSubscription = _firestore
      ...
      if (snapshot.docs.isNotEmpty) {
        ...
        print('🔄 Updated ${notifications.length} notifications from Firestore and cached');
      }
    }, onError: (error) {
  ...
  static Stream<List<Map<String, dynamic>>> getActivitiesStream({int limit = 10}) {
    ...
    if (cachedActivities != null && cachedActivities.isNotEmpty) {
      ...
      print('⚡ Emitted ${cachedActivities.take(limit).length} activities from cache INSTANTLY');
    } else {
    ...
    _activitiesSubscription = _firestore
      ...
      if (snapshot.docs.isNotEmpty) {
        ...
        print('🔄 Updated ${activities.length} activities from Firestore and cached');
      }
    }, onError: (error) {
  ...
  static Future<void> _fetchFreshStatistics() async {
    try {
      ...
      final submittedCount = await _firestore.collection('reports')
          .where('status', isEqualTo: 'submitted').count().get();
      final underReviewCount = await _firestore.collection('reports')
          .where('status', isEqualTo: 'under_review').count().get();
      final resolvedCount = await _firestore.collection('reports')
          .where('status', isEqualTo: 'resolved').count().get();
