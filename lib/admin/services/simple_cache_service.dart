...
class SimpleCacheService {
  ...
  static void _logCacheStatus() {
    ...
    print('📦 Cache Status:');
    print('  - Reports: ${reportsCount > 0 ? '$reportsCount items' : 'EMPTY'} ${_getCacheAge(_reportsTimestampKey)}');
    print('  - Notifications: ${notificationsCount > 0 ? '$notificationsCount items' : 'EMPTY'} ${_getCacheAge(_notificationsTimestampKey)}');
    print('  - Activities: ${activitiesCount > 0 ? '$activitiesCount items' : 'EMPTY'} ${_getCacheAge(_activitiesTimestampKey)}');
  }
  ...
  static Future<void> cacheReports(List<Map<String, dynamic>> reports) async {
    try {
      ...
      final serialized = reports.map((report) => _serializeFirestoreData(report)).toList();
  ...
  static Future<void> cacheNotifications(List<Map<String, dynamic>> notifications) async {
    try {
      final serialized = notifications.map((n) => _serializeFirestoreData(n)).toList();
      final jsonString = jsonEncode(serialized);
  ...
  static List<Map<String, dynamic>>? getCachedNotifications() {
    try {
      ...
      final decoded = jsonDecode(jsonString) as List;
      final notifications = decoded.map((item) => 
        _deserializeFirestoreData(Map<String, dynamic>.from(item))
      ).toList();
  ...
  static Future<void> cacheActivities(List<Map<String, dynamic>> activities) async {
    try {
      final serialized = activities.map((a) => _serializeFirestoreData(a)).toList();
      final jsonString = jsonEncode(serialized);
  ...
  static List<Map<String, dynamic>>? getCachedActivities() {
    try {
      ...
      final decoded = jsonDecode(jsonString) as List;
      final activities = decoded.map((item) => 
        _deserializeFirestoreData(Map<String, dynamic>.from(item))
      ).toList();
